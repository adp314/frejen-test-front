...
const useGetAllRequests = (token: string): UseGetAllRequestsResult => {
  return useQuery({
    queryKey: ["getAllRequestsData"],
    queryFn: async () => {
      try {
        const response = await api.authorized(token).get(`/all-requests`);
...
export default function Requests() {
  ...
  const { data: requestData } = useGetAllRequests(token);
          ...
          <div className="flex flex-col rounded-xl bg-maravilhas-white-100 mt-14">
            {filteredAndSortedRequests &&
              filteredAndSortedRequests.map((item: RequestType, index: number) => (
                <Link to={`/storage-manager/requests/${item && item.id}`} key={index} className="bg-maravilhas-black-100 bg-opacity-0 hover:bg-opacity-5 ">
                  <RequestCard data={item && item} withDetails={false} isStorageManager={true} />
